...
from . import hlo_helpers
# TODO(necula): roll up broader use of hlo_helpers.
custom_call = hlo_helpers.custom_call
...
def geev_hlo(dtype, a, *,
             input_shape_vals: Tuple[ir.Value, ...],  # a.shape as ir.Values
             jobvl=True, jobvr=True):
  # input_shape_vals are used for when a has dynamic shapes.
  _initialize()
  input_shape = ir.RankedTensorType(a.type).shape
  ...
  if dtype == np.float32:
    ...
    workspace_types = [ir.RankedTensorType.get([n, n], ir.F32Type.get())] * 3
    workspace_result_shapes = [hlo_helpers.shape_tensor((n_val, n_val))] * 3
    workspace_layouts = [[0, 1]] * 3
    eigval_types = [
        ir.RankedTensorType.get(batch_dims + (n,), ir.F32Type.get())] * 2
    eigval_result_shapes = [
        hlo_helpers.shape_tensor(batch_dims_vals + (n_val,))] * 2
    eigvals_layouts = [tuple(range(num_bd, -1, -1))] * 2
  elif dtype == np.float64:
    fn = b"lapack_dgeev"
    real = True
    eigvecs_type = ir.ComplexType.get(ir.F64Type.get())
    workspace_types = [ir.RankedTensorType.get([n, n], ir.F64Type.get())] * 3
    workspace_result_shapes = [hlo_helpers.shape_tensor((n_val, n_val))] * 3
    workspace_layouts = [[0, 1]] * 3
    eigval_types = [
        ir.RankedTensorType.get(batch_dims + (n,), ir.F64Type.get())] * 2
    eigval_result_shapes = [
        hlo_helpers.shape_tensor(batch_dims_vals + (n_val,))] * 2
    eigvals_layouts = [tuple(range(num_bd, -1, -1))] * 2
  elif dtype == np.complex64:
    ...
    eigvecs_type = ir.ComplexType.get(ir.F32Type.get())
    workspace_types = [
        ir.RankedTensorType.get([n, n], ir.ComplexType.get(ir.F32Type.get())),
        ir.RankedTensorType.get([2 * n], ir.F32Type.get())]
    workspace_result_shapes = [
        hlo_helpers.shape_tensor((n_val, n_val)),
        hlo_helpers.shape_tensor((hlo.AddOp(n_val, n_val).result,))]
    workspace_layouts = [[0, 1], [0]]
    eigval_types = [
        ir.RankedTensorType.get(batch_dims + (n,),
                                ir.ComplexType.get(ir.F32Type.get()))]
    eigval_result_shapes = [
        hlo_helpers.shape_tensor(batch_dims_vals + (n_val,))]
  ...
  batch_size_val = hlo_helpers.ir_constant(np.array(1, dtype=np.int32))
  for b_v in batch_dims_vals:
    logging.info(f"XXX {b_v.type=} and {batch_size_val.type=}")
  ...
  if any(a == ir.ShapedType.get_dynamic_size() for a in input_shape):
    result_shapes = workspace_result_shapes + eigval_result_shapes + [
        hlo_helpers.shape_tensor(input_shape_vals),
        hlo_helpers.shape_tensor(input_shape_vals),
        hlo_helpers.shape_tensor(batch_dims_vals),
    ]
  else:
    result_shapes = None
  out = custom_call(
      fn,
      result_types,
      [batch_size_val, n_val, _hlo_u8(jobvl_c), _hlo_u8(jobvr_c), a],
      operand_layouts=[scalar_layout] * 4 + [layout],
