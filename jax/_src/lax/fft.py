...
def _fft_lowering_cpu(ctx, x, *, fft_type, fft_lengths):
  ...
  if fft_type == xla_client.FftType.RFFT:
    ...
    assert shape[-len(fft_lengths):-1] == fft_lengths[:-1], (shape, fft_lengths)
    assert (out_shape[-1] // 2 + 1) == shape[-1], (out_shape, shape)
  ...
  assert len(shape) == len(out_shape) == ndims
  if out_dtype == np.float32:
    out_type = mlir.ir.F32Type.get()
  elif out_dtype == np.float64:
    out_type = mlir.ir.F64Type.get()
  elif out_dtype == np.complex64:
    out_type = mlir.ir.ComplexType.get(mlir.ir.F32Type.get())
  elif out_dtype == np.complex128:
    out_type = mlir.ir.ComplexType.get(mlir.ir.F64Type.get())
  else:
  ...
  size_fft_length_sum = np.prod(fft_lengths) if fft_lengths else 1
  size_fft_lengths = mlir.eval_dynamic_shape(ctx, (size_fft_length_sum,))[0]
  if isinstance(size_fft_lengths, int):
    size_fft_lengths = mlir.ir_constant(np.float64(size_fft_lengths),
  ...
  shape = mlir.shape_tensor(mlir.eval_dynamic_shape(ctx, shape))
  out_shape = mlir.shape_tensor(mlir.eval_dynamic_shape(ctx, out_shape))
  shape = shape if fft_type != xla_client.FftType.IRFFT else out_shape

  result_type = mlir.aval_to_ir_type(out_aval)
  return [ducc_fft.dynamic_ducc_fft_hlo(
      result_type,
      x, out_shape, shape, strides_in, strides_out, scale,
      x_aval.dtype, ndims=ndims, fft_type=fft_type,
      fft_lengths=fft_lengths)]
